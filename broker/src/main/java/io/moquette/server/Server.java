...
public class Server {
    ...
    public static void main(String[] args) throws IOException {
        ...
        server.startClusterClient();
        System.out.println("ClusterClient started");
        ...
        Runtime.getRuntime().addShutdownHook(new Thread() {
            @Override
            public void run() {
                server.stopClusterClient();
            }
        });
    }

    private void startClusterClient() {
        m_clusterClient.start();
    }

    private void stopClusterClient() {
        System.out.println("Shutdown Cluster Client");
        m_clusterClient.stop();
    }
    ...
    public void startServer(IConfig config, List<? extends InterceptHandler> handlers,
        ...
        m_clusterClient = new ClusterClient(this);

        m_acceptor = new NettyAcceptor();
        m_acceptor.initialize(processor, config, sslCtxCreator);
        m_processor = processor;
        m_processor.setClusterClient(m_clusterClient);
        m_initialized = true;
    }
